...
public:
    ...
    RobotSimServer() : rclcpp::Node("robot_sim_server")
    {
        ...
        map_dir.create(MAP_MIN_SIZE, MAP_MIN_SIZE, CV_8SC1);
        map_open.create(MAP_MIN_SIZE, MAP_MIN_SIZE, CV_32SC1);
        map_closed.create(MAP_MIN_SIZE, MAP_MIN_SIZE, CV_8SC1);
        path.clear();
...
private:
    ...
    cv::Mat map_dir, map_open, map_closed; // Used for path finding algorithm
    ...
    bool load_map(std::string map_path, unsigned int threshold, float resolution, bool flip, unsigned int rotate)
    {
        ...
        map_dir.create(img.size(), CV_8SC1);
        map_open.create(img.size(), CV_32SC1);
        map_closed.create(img.size(), CV_8SC1);
        path.clear();
    ...
    {
        (void)uuid; (void)goal; // to avoid unused parameter warnings
    ...
    bool navigate_to_pose(int target_x, int target_y, float speed, const std::shared_ptr<NavigateToPoseGoal> goal_handle = NULL)
    {
        ...
        float x_map, y_map;
        {
            ...
            compute_path(robot_x, robot_y, target_x, target_y);
            if (path.size() == 0)
            {
    ...
    void compute_path(int x0, int y0, int x1, int y1)				
    {
        path.clear();
        ...
        static std::priority_queue<Node> q[2]; // priority queue of open (not-yet-checked-out) nodes

        map_dir = cv::Scalar::all(-1); // map of directions to parent node
        map_closed = cv::Scalar::all(0); // map of closed (checked-out) nodes
        map_open = cv::Scalar::all(0); // map of open (not-yet-checked-out) nodes
        ...
        while(!q[qi].empty()) 
        {
            ...
            if(x == x1 && y == y1) // stop searching when the goal state is reached
            {
                ...
                path.reserve(rev_path.size() + 1);
                path.push_back(cv::Point(x0, y0)); // add start point
                for(int i = rev_path.size()-1; i >= 0; i-- )
                    path.push_back(rev_path.at(i));

                while(!q[qi].empty()) 
                    q[qi].pop(); // empty the leftover nodes
        ...
        if (path.size() == 0)
            ...
            RCLCPP_INFO(this->get_logger(), "Computed path of length %d in %d runs that took %f milliseconds", (int)path.size(), runs, duration.count());
    }
